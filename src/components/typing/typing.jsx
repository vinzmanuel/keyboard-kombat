...
import './styles.css'; // Assuming your CSS is stored in 'styles.css'
...
const TypingTest = () => {
    ...
    const mistakeIndices = useRef(new Set());
    const intervalRef = useRef(null);

    // Load a paragraph and reset states
    const loadParagraph = () => {
    const text = paragraphs[Math.floor(Math.random() * paragraphs.length)];
    setActiveParagraph(text.split(""));
    setCharIndex(0);
    setMistakes(0);
    setTotalTyped(0);
    setTimeLeft(60);
    ...
    const handleInput = (e) => {
        ...
        if (charIndex >= activeParagraph.length || timeLeft <= 0) return;
    
        let newTotal = totalTyped + 1;
        ...
        if (typedChar == null) {
            ...
            if (charIndex > 0) {
                ...
                // Reset the mistake status if the user deletes a wrong character
                if (mistakeIndices.current.has(charIndex - 1)) {
                    mistakeIndices.current.delete(charIndex - 1);
                    newMistakes -= 1;
                }
            }
            return;
            ...
            if (activeParagraph[charIndex] !== typedChar) {
                if (!mistakeIndices.current.has(charIndex)) {
                    newMistakes += 1;
                    mistakeIndices.current.add(charIndex);
                }
            } else {
                // If the user corrects the character, remove the mistake
                mistakeIndices.current.delete(charIndex);
            }
            setCharIndex(charIndex + 1);
        }
        
        const correct = newTotal - newMistakes;
        const acc = newTotal === 0 ? 100 : Math.round((correct / newTotal) * 100);
        const newWpm = Math.round((correct / 5 / (60 - timeLeft)) * 60);
        const newCpm = correct;
    ...
    return (
        <div className="wrapper">
            ...
            <div className="typing-text" onClick={() => inputRef.current.focus()}>
                {activeParagraph.map((char, idx) => {
                let className = '';
                if (idx === charIndex) className = 'active';
                if (idx < charIndex) {
                    className = mistakeIndices.current.has(idx) ? 'incorrect' : 'correct';
                }
            ...
            <div className="content">
                <ul className="result-details">
                    <li className="time">
                        <p>Time Left:</p>
                        <span className="text-yellow-200"><b>{timeLeft}</b>s</span>
                    </li>
                    ...
                    <li className="wpm">
                        <p>WPM:</p>
                        <span className="text-yellow-200">{wpm}</span>
                    </li>
                    <li className="cpm">
                        <p>CPM:</p>
                        <span className="text-yellow-200">{cpm}</span>
                    </li>
                </ul>
